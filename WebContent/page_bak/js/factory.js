...
app.controller("homeCtrl", function($scope, $http) {
	...
	$scope.getCustomerOrderList = function() {
		...
		$http.get(url1).success(function(response) {
			...
			//response = null;
			var orderInfoList = response.orderInfoList;
		  alert(orderInfoList.toString());
			var list = angular.fromJson(orderInfoList);
			 
			angular.forEach(orderInfoList,function(item,index,orderInfoList){
